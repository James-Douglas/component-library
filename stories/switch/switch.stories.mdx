...
<Preview>
  <Story name="switch">
    ...
    return (
      <DemoContainer>
        <Switch disabled />
        <Switch name={'checkedA'} label={'This is a Test'} checked={switches.checkedA} onChange={handleChange} />
        <Switch name={'checkedB'} label={'This is a Test'} checked={switches.checkedB} onChange={handleChange} labelVariant={'caption'} labelPlacement={'top'}/>
        <Switch name={'checkedC'} label={'This is a Test'} checked={switches.checkedC} onChange={handleChange} validationMessage={'This value is required'}/>
    </DemoContainer>
    );
    }}
  </Story>
...
<Story name="switch with props" decorators={[withKnobs]} parameters={{ docs: { disable: true } }}>
  {() => {
    ...
    const tooltip = object('Tooltip', { title: '', body: '' });
    ...
    return (
      <DemoContainer>
        <Switch id={id} name={name} label={label} labelVariant={labelVariant} tooltip={tooltip} checked={checked} onChange={handleChange} disabled={disabled} labelPlacement={labelPlacement} validationMessage={validationMessage} />
      </DemoContainer>
    );
  }}
</Story>
