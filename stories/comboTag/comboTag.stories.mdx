...
<Preview>
  <Story name="ComboTag preview" height="250px">
  {() => {
    const [comboTagValue, setComboTagValue] = useState('')
    const [apiData, setApiData] = useState([]);
    useEffect(() => {
      const filteredData = fakeApiData.filter((obj) => obj.name.toLowerCase().startsWith(comboTagValue.toLowerCase())).map(obj => obj);
             ...
             <Row>
               <Column>
                  <ComboTag
                    ...
                    keys={['name', 'code', 'alert', 'shortName']}
...
<Story name="ComboTag with props" decorators={[withKnobs]} parameters={{ docs: { disable: true } }}>
  {() => {
    ...
    const prefillValues = [{name: 'United States of America', code: 'US', alert: false, shortName: 'USA' }];
    ...
    const prefillValue = array('prefillValue', [], componentPropsId);
