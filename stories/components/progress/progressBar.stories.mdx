...
import { ProgressBar } from '@comparethemarketau/manor-progress';
...
Visually represents the current progress of a journey via a progress bar with a percentage value.

**Switch to canvas view to see examples**

<Story name="progress bar" parameters={{ docs: { disable: true } }}>
  <div style={{width: '100%'}}>
...
<Story name="progress bar demo" decorators={[withKnobs]} parameters={{ docs: { disable: true } }}>
  {() => {
    const value = number('value', 66, { range: true, min: 0, max: 100, step: 1 });
    return (
      <div>
